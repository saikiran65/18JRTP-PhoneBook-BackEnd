...
//		return findAllContacts();
		...
		List<Contact> entityList = contactRepository.findAll();
		//soft delete
		Contact delContact = entityList.stream().filter((ct) -> ct.getContactId()==id).findAny().get();
		delContact.setActiveSw("N");
		contactRepository.save(delContact);
		System.out.print(delContact);
